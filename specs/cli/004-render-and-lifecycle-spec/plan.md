...
**Branch**: `004-render-and-lifecycle-spec` | **Date**: 2026-01-26 | **Spec**: [./spec.md](./spec.md)
**Input**: Feature specification from `/specs/004-render-and-lifecycle-spec/spec.md`
...
- The renderer will operate on the `#ModuleRelease` definition. For local module paths, the CLI constructs the `#ModuleRelease` on the fly by:
  1. Loading the `#Module` from local path
  2. Unifying `values.cue` with any `--values` files
  3. Setting namespace from `--namespace` or `#Module.metadata.defaultNamespace`
  4. Setting name from `--name` or `#Module.metadata.name`

This ensures all component values are concrete/closed before rendering begins, simplifying the pipeline logic.
...
```text
specs/004-render-and-lifecycle-spec/
├── plan.md              # This file
