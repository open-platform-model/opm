...
```text
opm bundle publish <oci-ref> [flags]
```
...
| Argument | Required | Description |
|----------|----------|-------------|
| `<oci-ref>` | Yes | OCI registry reference for the bundle |

**Flags:**

| Flag | Short | Type | Default | Description |
|------|-------|------|---------|-------------|
| `--version` | `-v` | `string` | (required) | SemVer version tag |
| `--force` | `-f` | `bool` | `false` | Overwrite existing version |
...
```sh
# Publish bundle
opm bundle publish registry.example.com/my-bundle --version v1.0.0
```

**Notes:**

- Validates bundle with `opm bundle vet` before publishing
- Packages all referenced modules
- Uses credentials from `~/.docker/config.json`

**Related:** `opm bundle get`
...
```text
opm bundle get <oci-ref>@<version> [flags]
```
...
| Argument | Required | Description |
|----------|----------|-------------|
| `<oci-ref>@<version>` | Yes | Full OCI reference with SemVer version |

**Flags:**

| Flag | Short | Type | Default | Description |
|------|-------|------|---------|-------------|
| `--output-dir` | `-o` | `string` | CUE cache dir | Directory to download to |

**Examples:**

```sh
# Download bundle
opm bundle get registry.example.com/my-bundle@v1.0.0
```

**Notes:**

- Downloads bundle and all referenced modules
- Updates `module.cue` dependencies
- Uses credentials from `~/.docker/config.json`

**Related:** `opm bundle publish`
