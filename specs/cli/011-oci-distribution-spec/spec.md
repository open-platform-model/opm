...
A Platform Operator wants to curate a catalog by consuming generic upstream modules (e.g., `registry.opmodel.dev/postgres@v2`) and extending them with organizational specifics (e.g., adding a mandatory `#Policy`, injecting a logging sidecar, or setting default labels) without forking the upstream code.
...
**Independent Test**: Create a module `internal-db` that imports `registry.opmodel.dev/postgres@v2`, adds a `#Policy`, and runs `opm mod build`. Verify the output YAML contains the postgres resources *plus* the policy enforcement results.
...
- **FR-012**: The CLI MUST support a `registries` field in `~/.opm/config.cue` that **replaces** the `config.registry` field from 002-cli-spec. This field defines a map of module prefixes to registry configurations. The CLI MUST translate this map into `CUE_REGISTRY` format for all CUE module operations. An empty string key (`""`) serves as the default registry for modules that don't match any prefix.
    
    **Registry Resolution Behavior**: CUE uses longest-prefix-matching to resolve module paths to registries. When multiple prefixes match a module path, the longest (most specific) prefix wins. This resolution is order-independent. The CLI MUST sort prefixes by length (descending, longest first) when generating `CUE_REGISTRY` output to ensure deterministic and reproducible builds. The default registry (empty key) is always appended last.
    
    **Configuration Style**: While order does not affect resolution behavior, it is RECOMMENDED to define registries from most specific to least specific (top to bottom) for readability and maintainability.
    ...
    config: {
        registries: {
            // Most specific prefix first (recommended style)
            "company.internal/critical": {
                url: "harbor-critical.internal/modules"
                insecure: true
            }
            ...
            // Default matching for the official opm CUE modules
            "opmodel.dev": {
                url: "registry.opmodel.dev"
            }
            // Default registry for modules without a matching prefix (empty key)
            "": {
                url: "registry.opmodel.dev"
            }
        }
    ...
    This translates to: `CUE_REGISTRY=company.internal/critical=harbor-critical.internal/modules+insecure,company.internal=harbor.internal/modules+insecure,opmodel.dev=registry.opmodel.dev,registry.opmodel.dev`

- **FR-013**: The `OPM_REGISTRY` environment variable MUST support the same format as `CUE_REGISTRY`: a comma-separated list of `[modulePrefix=]hostname[:port][/repoPrefix][+insecure]` entries. The CLI MUST pass this value directly to CUE operations via `CUE_REGISTRY`. Example:

    ```bash
    # Single default registry
    OPM_REGISTRY=localhost:5000

    # Multiple registries with prefix routing
    OPM_REGISTRY=registry.opmodel.dev,company.internal=harbor.internal/modules+insecure
    ```

- **FR-014**: Registry configuration precedence (highest to lowest):
    1. `--registry` flag (single registry override for the command)
    2. `OPM_REGISTRY` environment variable (`CUE_REGISTRY` format)
    3. `config.registries` from `~/.opm/config.cue` (translated to `CUE_REGISTRY` format)

### Breaking Changes from 002-cli-spec

- **BC-001**: The `config.registry` field (single string) is removed and replaced by `config.registries` (prefix-to-registry map). This enables multi-registry routing compatible with `CUE_REGISTRY` semantics.
- **BC-002**: `OPM_REGISTRY` now accepts `CUE_REGISTRY` format (comma-separated prefix routing) instead of a single URL. Single URL values remain valid as a default registry.
