...
```text
Without Scopes:
    componentA.#policies: {NetworkPolicy: ...}
    componentB.#policies: {NetworkPolicy: ...}
    componentC.#policies: {NetworkPolicy: ...}
...
### Scope vs Component Policies

| Aspect | Component Policy | Scope Policy |
|--------|------------------|--------------|
| **Target** | `target: "component"` | `target: "scope"` |
| **Application** | One component | Multiple components |
| **Use case** | Intrinsic constraints | Cross-cutting concerns |
| **Who defines** | Component author | Platform team |
...
To apply a scope to all components in the module, use `#allComponentsList` from the module:
...
2. **Given** a Policy with `target: "component"`, **When** added to `#Scope.#policies`, **Then** validation fails (target mismatch).

3. **Given** a Policy with `target: "module"`, **When** added to `#Scope.#policies`, **Then** validation fails (target mismatch).
...
| Case | Behavior |
|------|----------|
| Policy with `target: "component"` | Validation error |
| Policy with `target: "module"` | Validation error |
