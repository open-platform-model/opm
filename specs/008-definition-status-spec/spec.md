...
**Supersedes**: This specification supersedes the module-status subspec in `001-core-definitions-spec/subspecs/module-status.md`.
...
- **FR-027**: Existing modules using `#ModuleStatus` fields MUST continue to work (backward compatibility)
...
- **SC-001**: All existing modules using `#ModuleStatus` continue to work without modification after migration to `#Status`
...
- Backward compatibility with existing `#ModuleStatus` usage is required (all current fields exist in `#Status`)
...
- **Existing implementations**:
  ...
  - `#ModuleStatus` in `catalog/v0/core/module.cue`
